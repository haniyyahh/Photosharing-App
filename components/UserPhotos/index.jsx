...
function UserPhotos({ userId, photoId = null, advancedFeaturesEnabled }) {
  ...
  // Only sync once when photos and photoId are ready
  useEffect(() => {
  ...
  // Update URL on photo index change (advanced mode)
  useEffect(() => {
  ...
  // Handlers for previous/next photo
  ...
  // Advanced mode rendering (single photo + controls)
    ...
    <Grid container spacing={2}>
      {userPhotos.map((photo) => (
        <Grid item xs={12} sm={6} md={4} key={photo._id}>
          <Card>
            <CardMedia component="img" image={`/images/${photo.file_name}`} alt={`Photo ${photo._id}`} />
